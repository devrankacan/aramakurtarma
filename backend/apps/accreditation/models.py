...
class AccreditationLevel(models.Model):
    ...
    name = models.CharField("Seviye Adı", max_length=100)
...
class Certification(models.Model):
    ...
    code = models.SlugField("Kod", max_length=50, unique=True)
    name = models.CharField("Sertifika Adı", max_length=150)
...
class UserCertification(models.Model):
    ...
    certification = models.ForeignKey(
        Certification, on_delete=models.PROTECT, related_name="holders", verbose_name="Sertifika"
    )
    issued_at = models.DateField("Veriliş Tarihi")
    expires_at = models.DateField("Geçerlilik Bitiş Tarihi", null=True, blank=True)
    document_ref = models.CharField("Belge Referansı", max_length=255, blank=True)
...
class AccreditationRecord(models.Model):
    ...
    team = models.ForeignKey(
        Team, on_delete=models.CASCADE, related_name="accreditation_records", verbose_name="Ekip"
    )
    ...
    valid_from = models.DateField("Geçerlilik Başlangıcı", null=True, blank=True)
    valid_until = models.DateField("Geçerlilik Bitişi", null=True, blank=True)
    certifying_body = models.CharField("Denetleyen Kurum", max_length=200, blank=True)
